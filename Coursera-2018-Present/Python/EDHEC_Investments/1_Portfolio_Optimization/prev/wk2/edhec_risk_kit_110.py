...
def get_cnsmr_returns():
    """
    consumer goods and retail stocks combined data
    """
    cnsmr = pd.read_csv("data/Stock_data_7.csv", header=0,
                        index_col=0, parse_dates=True)
    cnsmr = cnsmr.pct_change()
    cnsmr = cnsmr.dropna()
    cnsmr.index = cnsmr.index.to_period('M')
    return cnsmr
...
def msr(riskfree_rate, er, cov):
    """
    input: risk free rate of return, expected return, covariance matrix
    output: weights for maximum sharpe ratio
    """
    n = er.shape[0]
    init_guess = np.repeat(1/n, n)
    bounds = ((0.0, 1.0),) * n # tuples
